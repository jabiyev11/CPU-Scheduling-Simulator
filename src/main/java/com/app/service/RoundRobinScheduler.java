...
public final class RoundRobinScheduler implements Scheduler {
    ...
    public SchedulerResult schedule(List<Process> processes) {
        ...
        int time = 0;
        // Processing future arrivals in ascending arrival time
        ...
        while (index < processes.size() || !queue.isEmpty()) {
            ...
            if (!queue.isEmpty()) {
                ...
                // Merge adjacent slices of the same PID to keep Gantt output compact
                ...
                if (process.getRemainingTime() > 0) {
                    // Preempt current process. Quantum expired, move it to queue tail
                ...
                // No ready process at this time unit
                time++;
            }
        }
        return SchedulerMetricsCalculator.buildResult(SchedulingAlgorithm.ROUND_ROBIN, processes, scheduleLog);
    }
}
